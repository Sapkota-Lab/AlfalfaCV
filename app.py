...
def server(input_data, output, _session):
    ...
    @reactive.Effect
    @reactive.event(input_data.process)
    def process_image():
        """Process the uploaded image when the button is clicked."""
        nonlocal overlay, summary, diagnostics
        ...
        # Get parameters
        seed_type = input_data.seed_type()
        if seed_type == "bisected":
            mm_per_pixel = .00811
        else:
            mm_per_pixel = input_data.mm_per_pixel()

        min_area_px = input_data.min_area_px()
        min_area_mm2 = input_data.min_area_mm2()
        max_area_mm2 = input_data.max_area_mm2()
    ...
    def statistics_panel():
        ...
        if seed_type == "bisected":
            ...
            diagnostic_note = ""
            if diagnostics_data.get("area_filter_removed_all"):
                diagnostic_note = (
            ...
            elif diagnostics_data and diagnostics_data.get("prediction_count") == 0:
                diagnostic_note = (
